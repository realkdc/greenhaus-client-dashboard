...
// Extract key frames description from video (simplified approach using Gemini)
export async function extractVideoFramesDescription(
  videoBuffer: Buffer,
  fileName: string,
  mimeType: string
): Promise<{
  ...
  try {
    ...
    const prompt = `Analyze this video and describe 3-5 key moments or frames that best represent the content.
For each moment, briefly describe what's visible and when it occurs in the video.
Also estimate the video duration if possible.
Format your response as a clear, concise summary.`;
    ...
    return {
      description: text || "Video content analyzed",
    };
  } catch (error: any) {
    console.error("Error extracting video frames with Gemini:", error);
